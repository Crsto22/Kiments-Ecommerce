...
const paymentMethods = [
  ...
  },
  {
    ...
    type: "bcp" as const,
    detailNumber: "19192757772016",
    detailCci: "00219119275777201655",
...
export default function PagoPage() {
  ...
  const [warningModalTitle, setWarningModalTitle] = useState("Completa los campos");
  const [warningModalDescription, setWarningModalDescription] = useState("Revisa lo siguiente para continuar:");
  ...
  const validateStep1 = useCallback((): boolean => {
    ...
    if (requiresDniForReceipt && !docNumber.trim()) {
      newErrors.docNumber = "Ingresa tu DNI para boletas desde S/ 700";
    } else if (docNumber.trim() && !/^\d{8}$/.test(docNumber.trim())) {
    ...
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      setWarningModalTitle("Completa los campos");
      setWarningModalDescription("Revisa lo siguiente para continuar:");
      setShowWarningModal(true);
      return false;
    }
    setErrors({});
    return true;
  }, [docNumber, firstName, lastName, phone, email, wantsInvoice, rucNumber, shippingType, address, departamento, provincia, distrito, shippingRate, requiresDniForReceipt]);
  ...
  const copyPaymentValue = (value?: string) => {
    ...
    window.setTimeout(() => setCopiedPaymentValue(""), 1600);
  };

  const downloadYapeQr = async () => {
    try {
      const response = await fetch(YAPE_QR_URL);
      const blob = await response.blob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = "qr-yape-kiments.png";
      link.click();
      URL.revokeObjectURL(url);
    } catch {
      window.open(YAPE_QR_URL, "_blank", "noopener,noreferrer");
    }
  };
        ...
        <div className="flex flex-col items-center animate-[checkout-success-pop_500ms_ease-out]">
          ...
          <div className="mt-4 w-full max-w-xs rounded-lg border border-black/10 bg-[#fafafa] p-4 text-left">
            ...
            <p className="mt-3 text-[13px] leading-relaxed text-black/60">
              Pronto recibiras un correo electronico de confirmacion.
            </p>
                  ...
                  <div className="mt-4 flex items-start gap-4 rounded-md border border-[#dbe3f0] bg-white p-3">
                    ...
                    <div className="min-w-0 pt-1 text-left">
                      <div className="flex items-center gap-2 text-[#0057ff]">
                        <DeviceMobileSpeaker size={15} weight="bold" className="shrink-0" />
                        <p className="text-[12px] font-semibold">¿Tienes el comprobante en tu celular?</p>
                      </div>
                      <p className="mt-3 text-[12px] leading-relaxed text-black/70">
                        Escanea este QR para abrir esta misma página en tu teléfono y subir la foto directamente desde tu galería o cámara.
                      </p>
                    </div>
                  </div>
                )}
            ...
            {step === 1 && (
              <form className="space-y-10" onSubmit={(e) => { e.preventDefault(); handleGoToStep2(); }}>
                ...
                <section>
                  ...
                  <div>
                    <label className="mb-1.5 block text-[13px] font-light text-black/60">
                      <DeviceMobileSpeaker size={14} className="inline mr-1.5 -mt-0.5" />
                      Teléfono
                    </label>
                ...
                <section>
                  ...
                  <p className="mb-5 text-[13px] font-light text-black/55">
                    {requiresDniForReceipt
                      ? "Para boletas desde S/ 700 necesitamos tu DNI. Si deseas factura, puedes identificarte con RUC."
                  ...
                  <div className="grid grid-cols-[72px_1fr] overflow-hidden rounded-md border border-gray-300 bg-white focus-within:border-black focus-within:ring-1 focus-within:ring-black">
                    <div className="flex items-center justify-center border-r border-gray-300 bg-[#fafafa]">
                      <span className="text-sm font-light text-black/60">
                        DNI{requiresDniForReceipt ? " *" : ""}
                      </span>
                    </div>
                    <input
                      aria-label={requiresDniForReceipt ? "Número de DNI obligatorio" : "Número de DNI opcional"}
                      placeholder={requiresDniForReceipt ? "Número de DNI (obligatorio)" : "Número de Documento (opcional)"}
                      aria-required={requiresDniForReceipt}
                      value={docNumber}
                      onChange={(e) => {
                  ...
                  {requiresDniForReceipt && !errors.docNumber && (
                    <p className="mt-1.5 text-[11px] text-black/45">
                      Para boletas desde S/ 700 necesitamos identificarte con DNI.
                    </p>
                  )}
                </section>
              ...
              <div className="space-y-8">
                ...
                <section className="rounded-lg border border-black bg-black p-5 text-white sm:p-6">
                  ...
                  <p className="mt-2 text-[13px] text-white/70">
                    Realiza el pago exacto por {activePaymentMethod === "YAPE" ? "Yape" : "transferencia bancaria"}.
                  </p>
                </section>
                ...
                <section>
                  ...
                  {activePaymentMethod === "YAPE" ? (
                    <div className="rounded-lg bg-[#782080] px-4 py-5 text-center text-white sm:px-5">
                      <div className="mx-auto flex max-w-[420px] flex-col items-center gap-8 lg:flex-row lg:items-center lg:justify-center lg:text-left">
                        ...
                        <div className="max-w-[190px] text-center lg:text-left">
                          ...
                          <button
                            ...
                            className="mt-4 inline-flex h-9 items-center justify-center rounded-md border border-white/25 bg-white/10 px-3 text-[12px] font-medium text-white transition-colors hover:bg-white/20"
                          >
                            Descargar QR
                          </button>
                        </div>
                    ...
                    <div className="rounded-lg border border-gray-200 bg-[#fafafa] p-5 sm:p-6">
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="flex flex-col gap-1 border-b border-black/5 pb-2 sm:flex-row sm:items-center sm:justify-between">
                          <span className="text-black/60">Cuenta BCP Soles</span>
                          <span className="flex items-center gap-2 sm:justify-end">
                            ...
                            <button
                              ...
                              className="rounded p-1 text-black/45 transition-colors hover:bg-black/5 hover:text-black"
                              aria-label="Copiar cuenta BCP Soles"
                            >
                        ...
                        <div className="flex flex-col gap-1 border-b border-black/5 pb-2 sm:flex-row sm:items-center sm:justify-between">
                          <span className="text-black/60">Cuenta interbancaria</span>
                          <span className="flex items-center gap-2 sm:justify-end">
                            <span className="font-medium tracking-wider">{selectedPayment?.detailCci}</span>
                            <button
                              type="button"
                              onClick={() => copyPaymentValue(selectedPayment?.detailCci)}
                              className="rounded p-1 text-black/45 transition-colors hover:bg-black/5 hover:text-black"
                              aria-label="Copiar cuenta interbancaria"
                            >
                              {copiedPaymentValue === selectedPayment?.detailCci ? (
                                <CheckCircle size={16} weight="fill" className="text-green-600" />
                              ) : (
                                <Copy size={16} />
                              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 mx-6 w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl animate-[checkout-success-pop_300ms_ease-out]">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="mb-1 text-lg font-medium text-[#222]">{warningModalTitle}</h3>
              <p className="mb-4 text-[13px] text-black/50">{warningModalDescription}</p>
